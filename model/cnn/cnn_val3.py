...
class Net3(nn.Module):
    def __init__(self, INPUT_SIZEINPUT_SIZE):
        ...
        self.out = nn.Sequential(nn.Linear(64, 64),
                                 nn.ReLU(True),
                                 nn.Linear(64, 64),
...
model = torch.load('D:\home\zeewei\projects\\77GRadar\model\cnn\model_dir\model_cnn3\cnn3_pg1380_50.pkl')

# data_extractor = feature_extractor.FeatureExtractor()  # 此处全使用默认的文件路径配置
input_data = np.load('D:\home\zeewei\projects\\77GRadar\model\cnn\\test_data_pg\\input_data_avg_50_all.npy')
label_data = np.load('D:\home\zeewei\projects\\77GRadar\model\cnn\\test_data_pg\\input_data_avg_50_all.npy')
# data_extractor = feature_extractor.FeatureExtractor()  # 此处全使用默认的文件路径配置
...
for step in range(len(input_data)):
    ...
    if (abs(max_y_index - max_predict_index) < 10):
