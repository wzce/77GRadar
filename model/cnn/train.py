...
def train(model, model_save_dir, epochs=1000, save_line=0.7, learn_rate=LR):
    train_data_input, train_data_label, test_data_input, test_data_label = radar_data.load_pg_data_by_range(15, 30)
    ...
    for i in range(epochs):
        ...
        if i % 50 == 0:
            ...
            if test_loss < save_line:
                torch.save(model, model_save_dir + 'cnn_' + str(i) + '.pkl')
...
if __name__ == '__main__':
    cnn_model_dir = 'D:\home\zeewei\projects\\77GRadar\model\cnn\model_dir\cnn3\\'
    model = cnn_model.RadarCnn3().cuda(0)
    epochs = 300000
    train(model, cnn_model_dir, epochs, 0.6975, learn_rate=1e-4)
