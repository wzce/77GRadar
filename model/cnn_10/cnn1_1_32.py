...
MODEL_SAVE_DIR = 'D:\home\zeewei\projects\\77GRadar\model\cnn_10\model\cnn1_1_16\\'
...
def train_playground():
    ...
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\input_32_cnn1_1_32.npy', test_data_input_n)
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\label_32_cnn1_1_32.npy', test_data_label_n)

    test_batch_num = len(test_data_input)
    test_data_input = np.array(test_data_input_n).reshape(test_batch_num, 1, 32)
    test_data_label = np.array(test_data_label_n).reshape(test_batch_num, 1, 32)
    test_data_tensor = torch.FloatTensor(test_data_input).cuda(0)
    test_label_tensor = torch.FloatTensor(test_data_label).cuda(0)

    train_data_input = np.array(train_data_input_n).reshape(batch_num, 1, 32)
    train_data_label = np.array(train_data_label_n).reshape(batch_num, 1, 32)
    ...
    for i in range(3000):
        ...
        if i % 50 == 0:
            if test_loss < min_loss:
                min_loss = test_loss
            if test_loss < 0.70787:
                torch.save(model, MODEL_SAVE_DIR + 'cnn1_1_16' + str(i) + '_50.pkl')
