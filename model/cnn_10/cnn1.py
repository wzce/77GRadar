...
def train_playground():
    ...
    for item in train_data_label:
        v = decrease_range_resolution(item, 2)
    ...
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\input_data_cnn1_all_32.npy', test_data_input)
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\label_data_cnn1_all_32.npy', test_label_n)

    test_batch_num = len(test_data_input)
    test_data_input = np.array(test_data_input).reshape(test_batch_num, 1, 64)
    test_data_label = np.array(test_label_n).reshape(test_batch_num, 1, 32)
    ...
    train_data_label = np.array(train_label_n).reshape(batch_num, 1, 32)
    ...
    for i in range(3000):
        ...
        if i % 50 == 0:
            if test_loss < min_loss:
                min_loss = test_loss
            if test_loss < 0.6993:
                torch.save(model, MODEL_SAVE_DIR + 'cnn1_3_1' + str(i) + '_70.pkl')
