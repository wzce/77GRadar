...
model = torch.load('D:\home\zeewei\projects\\77GRadar\model\cnn_10\model\cnn3_1\cnn1_pg330_new_70_all2.pkl')

# data_extractor = feature_extractor.FeatureExtractor()  # 此处全使用默认的文件路径配置
input_data = np.load('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\input_data_avg_70_all_16.npy')
label_data = np.load('D:\home\zeewei\projects\\77GRadar\model\cnn_10\\test_data\\label_data_avg_70_all_16.npy')
# data_extractor = feature_extractor.FeatureExtractor()  # 此处全使用默认的文件路径配置
...
sca = [0 for i in range(16)]
sca_r = [0 for i in range(16)]
data_sca = [0 for i in range(16)]
for step in range(len(input_data)):
    ...
    predict = [j - j for j in range(0, 16)]
    predict[index] = 1
    predict = [predict]
...
right_distribute.distribute_cv(sca, data_sca, 16)
right_distribute.distribute_cv(sca_r, data_sca, 16)
