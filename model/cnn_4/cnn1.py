...
def train_playground():
    ...
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn\\test_data_pg\\input_data_avg_70_all.npy', test_data_input)
    np.save('D:\home\zeewei\projects\\77GRadar\model\cnn\\test_data_pg\\label_data_avg_70_all.npy', test_data_label)
    ...
    for i in range(3000):
        ...
        if i % 50 == 0:
            if test_loss < min_loss:
                min_loss = test_loss
            if test_loss < 0.6993:
                torch.save(model, MODEL_SAVE_DIR + 'cnn1_pg' + str(i) + '_new_70_all.pkl')
