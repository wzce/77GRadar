...
def train_with_pg_data(model, model_save_dir, epochs=3000, save_line=0.7, learn_rate=LR):
    ...
    test_data_input = np.array(test_data_input).reshape(test_data_num, 64, 1)
    test_data_label = np.array(test_data_label).reshape(test_data_num, 64, 1)
    ...
    train_data_input = np.array(train_data_input).reshape(train_data_num, 64, 1)
    train_data_label = np.array(train_data_label).reshape(train_data_num, 64, 1)
...
if __name__ == '__main__':
    model = rnn_model.Rnn3(INPUT_SIZE=1).cuda(0)
    print(model)
    cnn_model_dir = 'D:\home\zeewei\projects\\77GRadar\model\\rnn\model_save_dir\\rnn3\\'
    # model = rnn_model.RadarCnn3().cuda(0)
    epochs = 30000
    train_with_pg_data(model, cnn_model_dir, epochs, 0.7, learn_rate=1e-8)
